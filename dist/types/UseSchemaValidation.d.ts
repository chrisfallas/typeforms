...
export interface UseSchemaValidationProps<T extends Record<string, any> = Record<string, any>> {
    schemaValidation?: SchemaValidationFunction<T> | SchemaValidationObject<T>;
}
export interface UseSchemaValidationReturn<T extends Record<string, any> = Record<string, any>> {
...
export type SchemaValidationFunction<T extends object> = (values: Partial<T>) => Promise<SchemaValidationResult<T>> | SchemaValidationResult<T>;
export type SchemaValidationObject<T extends object> = Partial<{
    [K in KeyOf<T>]: (value: T[K] | undefined) => Promise<SchemaValidationError> | SchemaValidationError;
}>;
export type SchemaValidationResult<T extends Record<string, any> = Record<string, any>> = Partial<{
