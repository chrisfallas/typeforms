...
Select renders a select input (`<select>`) bound to a field in your schema. The type
of the field can be a string or a number, but the `options` prop must comply with the type.
