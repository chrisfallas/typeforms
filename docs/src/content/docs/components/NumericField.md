...
NumericField renders a number input (`<input type="number">`) and enforces that the form field is typed as a number.
It includes internal parsing and guards against empty or invalid numeric states.
This avoids common pitfalls of dealing with numeric inputs in React and ensures the submitted value is always a number, not a string or undefined.
